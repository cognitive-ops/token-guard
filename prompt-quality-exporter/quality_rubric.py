...
Vendored copy of prompt-intent-classifier/src/quality_rubric.py (kept in sync by
hand) so this exporter's Docker build context is self-contained, matching the
other exporters (they mount trained artifacts rather than importing classifier
source). Grades a single developer prompt against 4 best-practice dimensions
(1-5 each):
...
overall_score / tier are computed in Python from the 4 ints, not asked of the
model, to keep the schema enum-only (Anthropic structured-output limits: enums
+ array-of-enum only, no numeric/length constraints).
"""
